...
The setup consists of 2 parts. One for the Turtlebot4, the other for your local workstation.

For the turtlebot, you are required to `ssh ubuntu@<turtlebot4_ip_address>` and set it up from there. Either `git clone` or `scp` the files required.
Packages used for the Turtlebot4:
    1. `tb4_status`
    2. `my_tb4_nav`
    3. `tb4_status_interface`

For your local workstation, set up as per normal. Packages used for local workstation:
    1. `tb_fleet_server`
...
Before setting up the 3 packages, there are a few things to be done to the Turtlebot (assuming you have a reflashed turtlebot).
...
Before setting up the local workstation, ensure that your Turtlebot and local workstation are connected to the same network.
...
1. Mapping/Navigation Issues
    ...
    - __Getting an error/warning stating that controller frequency is not met (expected > actual)__
        - Error occurs due to CPU issues as well... (Frequency currently set to a low 8Hz)
